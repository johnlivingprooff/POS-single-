...
import { Scan, Plus, Minus, Trash2, ShoppingCart, CreditCard, X } from 'lucide-react';
...
const POSPage: React.FC = () => {
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 p-6 pb-28 lg:pb-6 bg-gray-50">
      ...
      <div className="hidden lg:flex flex-col p-6 bg-white border-l w-96">
      ...
      <div className="lg:hidden fixed inset-x-0 bottom-0 z-40 p-3 bg-white/95 backdrop-blur border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
        <button
          onClick={() => setShowCartSheet(true)}
          className="flex items-center justify-between w-full px-5 py-3 font-medium text-primary-foreground bg-primary rounded-full shadow-sm active:scale-[0.99] transition"
        >
          <span className="flex items-center gap-2">
            <ShoppingCart className="w-5 h-5" />
            <span className="font-semibold">{cart.length} {cart.length === 1 ? 'item' : 'items'}</span>
          </span>
          <span className="font-bold">{currency}{getTotal().toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </button>
      </div>
      ...
      {showCartSheet && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-2xl max-h-[85vh] flex flex-col animate-in slide-in-from-bottom">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-900">Your Cart ({cart.length})</h2>
              <button onClick={() => setShowCartSheet(false)} className="p-1 text-gray-500 hover:text-gray-900" aria-label="Close cart">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              {/* Customer Info */}
              <div className="mb-6">
                <label htmlFor="customer-m" className="block mb-2 text-sm font-medium text-gray-700">
                  Customer (Optional)
                </label>
                <select
                  id="customer-m"
              ...
              {cart.length > 0 && (
                <div className="pt-4 space-y-4 border-t">
                  <div>
                    <label htmlFor="discount-m" className="block mb-2 text-sm font-medium text-gray-700">
                      Discount (%)
                    </label>
                    <input
                      id="discount-m"
